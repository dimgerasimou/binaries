...
cscripts="time spacer mediacontrol bluetooth-menu"
xscripts="keyboard"
comscripts="battery date kernel bluetooth internet memory power volume"
shscripts=""
...
for script in $comscripts; do
	gcc $script.c common.c -o $script -Os -Wall -lm `pkg-config --cflags --libs libnotify`
	mv $script $path/
done
